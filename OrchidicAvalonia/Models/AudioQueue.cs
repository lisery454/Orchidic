...
public class AudioQueue
{
    ...
    {
        CurrentIndex = 0;
        _audioFiles = new SourceList<AudioFile>();
        _audioFiles.AddRange([
            ...
            new AudioFile(@"D:\Music\[Muse Dash] 恋爱回避依存症 - Matthiola Records.mp3"),
            new AudioFile(@"D:\Music\[ON YOUR GLORY DAY] わっふゑ - I'M ALIVE.mp3"),
            new AudioFile(@"D:\Music\「 Nightcore 」 → Kimi ni Tsutaetakute.mp3"), new AudioFile(
                @"D:\Music\【originalMV】自分後回し⧸P丸様。.mp3"),
            new AudioFile(@"D:\Music\【とあ】拼凑的断音【初音ミク】.mp3"),
            new AudioFile(@"D:\Music\[DJMAX RESPECT] Groovin Up - Mycin.T.mp3"),
            new AudioFile(@"D:\Music\[Lanota-Arcaea] Dream goes on - Tiny Minim .mp3"),
    ...
    public SourceList<AudioFile> _audioFiles;

    // ✅ 暴露可观察的只读序列（订阅者会收到每次变化后的完整列表）
    public IObservable<IReadOnlyCollection<AudioFile>> AudioFilesObservable =>
        _audioFiles.Connect().ToCollection();

    // ✅ 如果还想暴露为普通可读集合（便于内部访问）
    public IReadOnlyList<AudioFile> AudioFiles => _audioFiles.Items.ToList();
